...
Ext.namespace("GEOB");
	 ...
	 */
		...
		var describeProcess = function(url,identifier){			
			var wps = new OpenLayers.WPS(url, {onDescribedProcess: onDescribeProcess});		
		...
		*Parameter optional jobid integer, link with a Graphic Window 
		...
		var getColor = function(){
			...
			var nprofile = getGraphicHandler(); //drawLayer.features.length;
			var color;
			switch (nprofile){	
				case 1:
				case 2:
		...
		var onNewLine = function(e){
			...
			showLayers();
			var profileColor = getColor();
			var graphicHandler = getGraphicHandler();
			feature.style = {pointRadius: 10, fillColor: "green", fillOpacity: 0.5, strokeColor: '#' +profileColor};
			feature.attributes = {profile: graphicHandler,color:profileColor};
			drawLayer.redraw();
		...
		var getprofile = function(feature,option,parameters){
			var url = wps_url;			
			if (option == 'new') {
				wps = new OpenLayers.WPS(url,{onSucceeded: onExecuted,onFailed : onError});
				}
			else{
				wps = new OpenLayers.WPS(url,{onSucceeded: onUpdated,onFailed : onErrorUpdated});
			}			
			var InputGML = new OpenLayers.WPS.ComplexPut({identifier:"data",value:convertToGML(feature)});			
			var InputReferentiel = new OpenLayers.WPS.LiteralPut({identifier:"referentiel",value:parameters.referentiel.value,format:"Litteral"});
			var InputPas = new OpenLayers.WPS.LiteralPut({identifier:"distance",value:parameters.pas.value,format:"Litteral"});			
			var InputFormat = new OpenLayers.WPS.LiteralPut({identifier:"outputformat",value:'json',format:"Litteral"});
			var OutputInfos = new OpenLayers.WPS.LiteralPut({identifier:"resultinfos"});
			var OutputResult = new OpenLayers.WPS.LiteralPut({identifier:"result"});
						
			var profilprocess =  new OpenLayers.WPS.Process({identifier:wps_identifier,
		...
		var convert2csv = function (data){		
			var format = new OpenLayers.Format.JSON();
				OpenLayers.Request.POST({
					...
					data: format.write({columns:['distance','x','y','altitude','pente'], data: data}),
		...
		var updateChartConfig =function (jobid){
			...
			var mask_loader = new Ext.LoadMask(
				'configform' + jobid,				
				{msg:tr("addonprofile.update")}
			);
			mask_loaders.push({id:'loaderprofil' + jobid, loader: mask_loader});
			//mask_loader.id = 'loaderprofil' + jobid;
			mask_loader.show();
			var parameters = {pas:{value:Ext.getCmp('configform' + jobid).getForm().findField('pas').getValue()},
				referentiel:{value:Ext.getCmp('configform' + jobid).getForm().findField('referentiel').getValue()}};
			var feature = drawLayer.getFeaturesByAttribute('profile',parseInt(jobid));			
			...
			if (obj.profile.points.length > 0)
				{								
					...
					for (var i=0; i<mask_loaders.length; i++){
						if (mask_loaders[i].id == 'loaderprofil' +jobid)
							mask_loaders[i].loader.hide();
				...
				{	
					...
					lineChart = new Ext.chart.LineChart({
						...
						series: [{
							...
							yField: 'z',								
							style: {
								color: profileColor, // couleur de  la ligne
								lineSize: 1, //taille de la ligne	
								fillColor: profileColor,// couleurs des points														
								fillAlpha:0.8,// Opacité des points
								size: 4 // taille des points
													
							}
						}], 
						listeners: { itemmouseover: function(o){							
					...
					var infosForm = new Ext.FormPanel({  
						...
						buttons: [{			
							...
							handler: function(){ convert2csv(obj.profile.points);}}]
					});  					
					...
					var chartWindow = new Ext.Window({
						...
						pageX:10,
						pageY:Ext.getBody().getHeight()-(250 +(parseInt(jobid) *20)),
						resizable: true,
						width    : 600,
						//height   : 450,
						border : false,							
	 ...
	 */	
        ...
        create:function(m,wpsconfig){
			...
			var menuitems =	new Ext.menu.Item({text:wpsconfig.title,
					id:'wpsprofiletools',
					...
					menu: new Ext.menu.Menu({
						...
						if (initialized==false){describeProcess(wps_url,wps_identifier);}						
						}},
						items: [
