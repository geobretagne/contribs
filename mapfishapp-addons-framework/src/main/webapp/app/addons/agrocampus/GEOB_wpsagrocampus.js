...
Ext.namespace("GEOB");
	 ...
	 */
		...
		var getinfosmnt = function(lonlat){
			...
			var mntin = new OpenLayers.WPS.LiteralPut({identifier:"MNT Utilise",value:"Bretagne 50m",format:"Literal"});
			var epsgIn = new OpenLayers.WPS.LiteralPut({identifier:"EPSG IN",value:"epsg:2154 (Lambert 93)",format:"Literal"});
			var X = new OpenLayers.WPS.LiteralPut({identifier:"x",value:parseInt(lonlat.lon),format:"Literal"});
			var Y = new OpenLayers.WPS.LiteralPut({identifier:"y",value:parseInt(lonlat.lat),format:"Literal"});
			
			var stdout = new OpenLayers.WPS.LiteralPut({identifier:"stdout",format:"Complex"});
            
            var infomnt =  new OpenLayers.WPS.Process({identifier:wps_identifier,
			...
			var menuitems =	new Ext.menu.Item({
					...
					menu: new Ext.menu.Menu({						
						items: [
						{
                            iconCls:'wps-infomnt',
							text: 'infos MNT',							
                            handler: function(){ click.activate();}
                        }]						
						})});			
			return menuitems;
		}
